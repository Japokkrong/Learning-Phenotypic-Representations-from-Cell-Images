...
def extract_feature_pipeline(args, weights, channel):
    ...
    # ============ building DINOv3 network ... ============
    print(f"Creating DINOv3 model: {args.arch}")
    backbone = timm.create_model(args.arch, pretrained=False, num_classes=0)
    embed_dim = backbone.num_features
    print(f"Embedding dimension: {embed_dim}")
    
    # For evaluation, we only need the backbone (no head needed)
    model = backbone
    model.cuda()
    ...
    # Load teacher weights (the EMA model from training)
    if args.checkpoint_key in checkpoint:
        state_dict = checkpoint[args.checkpoint_key]
        # Remove 'module.' prefix if present (from DDP)
        state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        # Remove 'backbone.' prefix to match the model structure
        state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items() if 'backbone' in k}
        model.load_state_dict(state_dict, strict=False)
        print(f"Loaded {args.checkpoint_key} weights from checkpoint")
    else:
        print(f"Warning: '{args.checkpoint_key}' not found in checkpoint. Available keys: {checkpoint.keys()}")
    ...
    if args.dump_features and dist.get_rank() == 0:
        ...
        torch.save(train_features.cpu(), os.path.join(args.dump_features, f"021_trainfeat_dinov3.pth"))
        train_features_cpu = train_features.cpu()
        features_np = train_features_cpu.numpy()
        df_csv = pd.DataFrame(features_np)
        df_csv.to_csv("021_trainfeatures_dinov3.csv", index=True)
        
    return train_features, train_features2
...
def extract_features(model, data_loader, use_cuda=True):
    ...
    for samples, index, compound, moa, replicate, treatment, plate, batch in metric_logger.log_every(data_loader, 10):
        ...
        for samp in range(4):
            ...
            # Forward through backbone and get pooled features
            feats_hold = model.forward_features(a)
            # Apply global average pooling over sequence dimension (keeping embed_dim)
            if len(feats_hold.shape) == 3:  # [batch, seq_len, embed_dim]
                feats_hold = feats_hold.mean(dim=1)  # [batch, embed_dim]
...
@torch.no_grad()
def extract_features2(model, data_loader, use_cuda=True):
    metric_logger = utils.MetricLogger(delimiter="  ")
    features = None
...
def Aggregate_features_NSCB(features, channel, epoch):
    ...
    df.to_csv(f"NSCB_aggregated_features_DINOv3_{channel}_epoch_{epoch}.csv", index=True)
    return df
...
class ReturnIndexDataset(Dataset):
    def __init__(self, path0, channel):
        ...
        self.aug1 = albumentations.Compose([
            albumentations.augmentations.crops.transforms.Crop(x_min=64, y_min=32, x_max=320, y_max=256,
                                                                always_apply=True), ])
        self.aug2 = albumentations.Compose([
            albumentations.augmentations.crops.transforms.Crop(x_min=320, y_min=32, x_max=544, y_max=256,
                                                                always_apply=True), ])
        self.aug3 = albumentations.Compose([
            albumentations.augmentations.crops.transforms.Crop(x_min=64, y_min=256, x_max=320, y_max=480,
                                                                always_apply=True), ])
        self.aug4 = albumentations.Compose([
            albumentations.augmentations.crops.transforms.Crop(x_min=320, y_min=256, x_max=544, y_max=480,
                                                                always_apply=True), ])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('DINOv3 Evaluation on BBBC021')
    ...
    parser.add_argument('--arch', default='vit_small_patch16_dinov3.lvd1689m', type=str, 
                        help='DINOv3 architecture from timm')
    parser.add_argument("--checkpoint_key", default="teacher", type=str,
                        help='Key to use in the checkpoint (teacher or student)')
    parser.add_argument('--dump_features', default='features_dinov3',
                        help='Path where to save computed features')
    ...
    parser.add_argument('--channel_headers', default=['Image_FileName_DAPI', 'Image_FileName_Tubulin',
                                                       'Image_FileName_Actin'], type=list)
    ...
    tally_epoch = []
    for channel in [0]:  # DAPI channel
        print("channel:", channel)
        for train_epoch in range(180, 200, 5):
            ...
            print(f"Epoch {train_epoch} NSCB: {nscb_epoch}")
            print(f"All results so far: {tally_epoch}")
    
    print(f"\nFinal NSCB scores: {tally_epoch}")
    dist.barrier()
