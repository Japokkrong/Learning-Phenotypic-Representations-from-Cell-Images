# WS-DINO: Adaptation of DINO from facebook research available here:
...
for nums in weight_list:
  for val in nums:
    weight_list_flat.append(val)

weighty = 1/torch.tensor(weight_list_flat)
...
class NaturalImageDataset(Dataset):
    ...
    def __getitem__(self,i):
        ...
        rand_Aimage = Image.open(self.X0[random_i])
        rand_Aimage = np.array(rand_Aimage)
        rand_Aimage[rand_Aimage > 10000] = 10000
        
        for _ in range(self.local_crops_number):
                     
            transformed2 = self.aug_local_crop(image=rand_Aimage)
            image = transformed2['image']
            image_2 = image.astype(np.float32)      
