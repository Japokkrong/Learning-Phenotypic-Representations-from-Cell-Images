# Adaptation of DINO from facebook research available here:
# https://github.com/facebookresearch/dino/blob/main/main_dino.py
# 
# This version of the script will train the DAPI (DNA) channel. Change the channel heaeders and name of output scripts to train the other channels
#
# ImageNet weights (pretrain_full_checkpoint.pth in this script) for ViT-S/8 are taken from: 
...
class NaturalImageDataset(Dataset):
    ...
    def __getitem__(self,i):
        self.Aimage = Image.open(self.X0[i])
        self.Aimage = np.array(self.Aimage)
        self.Aimage[self.Aimage > 10000] = 10000

        self.transformed0 = self.aug0(image=self.Aimage)
        self.image = self.transformed0['image']
        self.image_0 = self.image.astype(np.float32)

        self.transformed1 = self.aug1(image=self.Aimage)
        self.image = self.transformed1['image']
        self.image_1 = self.image.astype(np.float32)
  
        self.image_0 = np.expand_dims(self.image_0,0)      
        self.image_1 = np.expand_dims(self.image_1,0)
     
        self.image_0 = np.concatenate((self.image_0, self.image_0, self.image_0),axis=0)
        self.image_1 = np.concatenate((self.image_1, self.image_1, self.image_1),axis=0)
                
        self.image_0 = torch.tensor(self.image_0, dtype=torch.float)
        self.image_1 = torch.tensor(self.image_1, dtype=torch.float) 
        self.crops = []
        self.crops.append(self.image_0)
        self.crops.append(self.image_1)
        
        for _ in range(self.local_crops_number):
                     
            self.transformed2 = self.aug_local_crop(image=self.Aimage)
            self.image = self.transformed2['image']
            self.image_2 = self.image.astype(np.float32)      
            self.image_2 = np.expand_dims(self.image_2,0)                
            self.image_2 = np.concatenate((self.image_2, self.image_2, self.image_2),axis=0)
            self.image_2 = torch.tensor(self.image_2, dtype=torch.float) 
            self.crops.append(self.image_2)
...
def train_dino(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        utils.save_on_master(save_dict, os.path.join(args.output_dir, 'DAPI_DINO_checkpoint.pth'))
        if args.saveckp_freq and epoch % args.saveckp_freq == 0:
            utils.save_on_master(save_dict, os.path.join(args.output_dir, f'DAPI_DINO_checkpoint{epoch:04}.pth'))
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     'epoch': epoch}
        if utils.is_main_process():
            with (Path(args.output_dir) / "DAPI_DINO_log.txt").open("a") as f:
